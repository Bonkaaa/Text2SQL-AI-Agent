# Kiến trúc AI Agent Text-to-SQL Self-Service Analytics (DATA-01) - v2.0
### Mô hình kết hợp: DeepAgent Harness (Suy luận linh hoạt) + LangGraph Control Pipeline (Kiểm soát tất định)
...
- **Mục tiêu hệ thống**: Xây dựng AI Agent có khả năng:
  ...
  5. Chạy trên Data Warehouse (BigQuery / DuckDB), trả kết quả dạng bảng + biểu đồ trực quan + diễn giải insight kinh doanh.
  6. Tự sửa lỗi truy vấn (Self-Correction Loop) có giới hạn và đo lường độ chính xác (Execution Accuracy).
...
1. **Tính linh hoạt (Agentic Reasoning)**: Hiểu ngôn ngữ tự nhiên tiếng Việt đa dạng, phát hiện ý định mơ hồ, chọn lọc schema phù hợp, suy luận cách join bảng.
...
- **LangGraph (Control Layer - Hàng rào kiểm soát tất định)**:
...
> **Tóm tắt luận điểm bảo vệ**: *"DeepAgents lo phần NGHĨ (linh hoạt theo ngôn ngữ tự nhiên), LangGraph lo phần KIỂM SOÁT (chặt chẽ, tất định, đảm bảo an toàn dữ liệu doanh nghiệp)"*.

---

## 3. Sơ đồ Kiến trúc Tổng thể (Architecture Flowchart)
...
- **Error-Aware Prompting**: Khi được kích hoạt từ luồng `RETRY_CHECK`, prompt sẽ được nạp bổ sung:
  ...
  - Chi tiết lỗi kỹ thuật từ node `ERR` (Lỗi cú pháp, cột cấm RBAC, hoặc thông báo lỗi từ database).
...
| **5. Warehouse Executor** | Client Driver (BigQuery / DuckDB) | - Thực thi câu lệnh an toàn, áp dụng query timeout (30s). |
| **6. Audit Logger** | Database Logging Table | - Luôn luôn chạy: Ghi nhận `query_id`, `user_id`, `role`, `question`, `sql`, `status`, `bytes_scanned`, `execution_time_ms`, `timestamp`. |

### 4.6. Vòng lặp Sửa lỗi có Giới hạn (Bounded Self-Correction)
- Biến trạng thái `retry_count` được theo dõi trong state graph.
- Ngưỡng tối đa: `MAX_RETRIES = 3`.
- Nếu vượt quá 3 lần: Pipeline dừng lại, trả thông báo lỗi thân thiện cho người dùng giải thích vì sao không thể thực hiện câu hỏi này, đồng thời lưu vết vào Audit log để đội ngũ kỹ thuật cải tiến prompt/few-shot.

### 4.7. Subagent: Response Synthesizer
...
## 5. Danh mục Tools phát triển

| Tên Tool | Thành phần sử dụng | Mô tả chức năng | Xử lý lỗi / Fallback |
|---|---|---|---|
...
| `persist_audit_log` | LangGraph Audit Node | Ghi vết truy vấn vào bảng audit log | Lưu log bất đồng bộ, không làm crash luồng chính nếu log thất bại |

---

## 6. Chiến lược Tối ưu Chi phí & Hiệu năng (Model Tiering)
...
```
       ...
       +------------------------+------------------------+
       |                                                 |
[ Tác vụ Suy luận Phức tạp ]               [ Tác vụ Rút gọn / Tiện ích ]
       |                                                 |
...
(Claude 3.5 Sonnet / GPT-4o)              (Claude 3.5 Haiku / GPT-4o-mini)
       |                                                 |
Độ chính xác cú pháp & logic cao            Tối ưu chi phí, độ trễ cực thấp
```

- **Tiết kiệm 100% chi phí token cho Governance Pipeline**: Toàn bộ 6 bước trong LangGraph Control Pipeline chạy bằng code Python thuần và API database, hoàn toàn không gọi LLM để validate hay check quyền.
...
| **Self-Correction Success Rate** | $\frac{\text{Số câu sửa thành công sau retry}}{\text{Số câu lỗi lần đầu}} \times 100\%$ | $\ge 60\%$ |
...
3. **Tuần 5 - 6: Cài đặt LangGraph Control Pipeline**
...
6. **Tuần 11 - 12: Đánh giá Eval, Tối ưu & Viết Báo cáo**
   - Chạy bộ benchmark 50 câu hỏi, đo Execution Accuracy.
